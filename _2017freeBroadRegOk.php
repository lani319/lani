...
/*
$testId = $_COOKIE['_CKE_USER_ID'];

if($testId != "sinan29"){

	popupMsg("현재 서비스 점검중에 있습니다.");
...
//무료회원인지 검출 (등급 설정)
$SQL1 = "SELECT count(*) as num  FROM `Member` where  level='10' and member_state='Y' AND (gradeLevel = '2' or gradeLevel = '3' ) AND userID='".$_COOKIE['_CKE_USER_ID']."'";
//echo $SQL1;
$Result = mysql_query($SQL1) or die(mysql_error());
$Rs = mysql_fetch_array($Result);
if($Rs['num'] <= 0 ){
	popupMsg("2등급, 3등급 회원만 신청 가능합니다.");
	echo "<script> history.back();</script>";
	exit();
}
...
//회원정보 취득
$SQL="select gradeLevel from Member where userID='".$_COOKIE['_CKE_USER_ID']."'";
//echo $SQL;
$result=mysql_query($SQL) or die(mysql_error());
$Member=mysql_fetch_array($result);
	...
	if($mLevel==2){
		...
		$options = "2등급 방송 2일 체험";
	}else if($mLevel==3){
		$enddate = mktime(23,59,59,$month,$day,2017);
		$options = "3등급 방송 1일 체험";
	}
