...
function couponUse5percent()
{
	...
	if ($MemLV == 2)
	{
		$sql = "select idx from 502Coupon where kind=$couponNo and status = 0 and memIdx=$memIdx";
		$tmpRs = mysql_query($sql);
...
function couponCheck35percentTh($memidx){
	...
	$sql = "select count(idx) from 502Coupon where kind=$couponNo and status = 0 and memIdx=$memIdx";		
	$tmpRs = mysql_query($sql);
	$rs = mysql_fetch_array($tmpRs);
...
function setCouponJongmokValue($memIdx,$num)
{
	/*
		...
		*/
		...
		$state = 6; //임의적용	
		//$num = 3;
		$bank="502쿠폰(적정주가 ".$num."회 이용권)";
		$jongmok=$num; //3개월
		$settleType=3; //임의적용
		$couponKind = "$num 개(등급 쿠폰)";
		
		$sql = "select idx from 502Coupon where kind=17 and isEvent = '1' and memIdx=$memIdx";
						
		$tmpRs = mysql_query($sql);
		$rs = mysql_fetch_array($tmpRs);
				
		if($rs[idx]== null)  //발행되었고, 사용된 쿠폰이 없으면
		{
			//회원정보에서 갱신
			...
			$sql = "insert into 502Coupon(kind,issueDate,expiredDate,memIdx,etc,isEvent) values(17,now(),'2024-12-31 23:59:59',$memIdx,'$couponKind','1')";
			mysql_query($sql) or die($sql);
